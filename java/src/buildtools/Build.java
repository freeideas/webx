...
public class Build {
    ...
    public static void compile() throws Exception {
        ...
        {
            Log.log( "removing all class files" );
            LibFile.rm( CLS_DIR );
            CLS_DIR.mkdirs();
        }
        {
            Log.log("compiling from entry point");
    ...
    public static File buildJar() throws Exception {
        ...
        Log.log( "Extracting dependencies from LIB_DIR to CLS_DIR" );
        File[] libFiles = LIB_DIR.listFiles();
        if ( libFiles != null ) {
            for ( File file : libFiles ) {
                if ( !file.getName().endsWith( ".jar" ) ) continue;
                try {
                    Log.log( "Extracting: " + file.getName() );
                    LibFile.unzip( file, CLS_DIR, null, null );
                } catch ( IOException e ) {
                    Log.log( "Error extracting " + file.getName() + ": " + e.getMessage() );
                }
            }
        }
        Log.log("Creating JAR file: " + LibFile.getCanonicalPath(jarFile));
    ...
    public static File buildExe( File jarFile ) throws Exception {
        ...
        if (jarFile == null || !jarFile.exists()) {
            Log.log("Failed to build JAR file");
            throw new Exception("Failed to build JAR file");
        }
        Log.log("Creating native executable: " + LibFile.getCanonicalPath(exeFile));
        ...
        if (result != 0) {
            Log.log("Native image creation failed with error code: " + result);
            Log.log("Check the log file for details: " + LibFile.getCanonicalPath(logFile));
            ...
            exeFile = destFile;
            Log.log("Native executable creation successful: " + LibFile.getCanonicalPath(exeFile));
        }
        return exeFile;
    }
    ...
    private static File srcFile( Class<?> cls ) {
        ...
        LibTest.asrt( srcFile.isFile() );
        return srcFile;
    }
    @SuppressWarnings("unused")
    private static boolean srcFile_TEST_( boolean findLineNumber ) {
        if (findLineNumber) throw new RuntimeException();
        LibTest.asrt( srcFile(Build.class).isFile(), "srcFile() should return a valid file" );
        return true;
    }
